//Board.cpp
//Implementation fo the Board class.
//Represents the game board. 
//Contains a 20x20 integer array.
// 0 = unoccupied, 1-4 corresponds to the player.
//Also contains logic to check for legal placement of pieces.
...
int Board::placePiece(Piece piece, int x, int y){ //todo: make this take in a move
...
//checks if a piece is adjacent to one of its same color (and therefor not legal)
	bool flag = false;
	if (onBoard(x - 1, y)) {
...
//Checks if a piece is diagonal to one of its same color (and thus legal).
	bool flag = false;
...
//records the board to a text file.
  for(int i=0;i<20;i++){
    for(int j=0;j<20;j++){
